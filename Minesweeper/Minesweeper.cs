    ...
    public partial class Minesweeper : Form
    {
        ...
        private void pbCanvas_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int x = 0; x < Settings.Width; x++)
            {
                ...
                {
                    if (game.grid[x, y].caseState == CaseState.Hidden)
                    { // square Hidden
            ...
            { // If Game isRunning()
                ...
                switch (e.Button)
                {
                    case (MouseButtons.Left):
                        ...
                        try
                        {
                            if (game.grid[MousePosX, MousePosY].caseType == CaseType.Mine)
                            { // isMine()
                                game.Loose();
                                game.showAllMines();
        ...
        private void setButtonDependencies()
        {
            ...
            var posTextBoxPlayerName = this.PointToScreen(TextBoxPlayerName.Location);
            posTextBoxPlayerName = pbPlayer1.PointToClient(posTextBoxPlayerName);
            TextBoxPlayerName.Parent = pbPlayer1;
            TextBoxPlayerName.Location = posTextBoxPlayerName;
